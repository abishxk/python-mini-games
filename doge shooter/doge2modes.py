...
def kills():
    game.init()
    mycursor.execute("SELECT MAX(Score) FROM doge")
    ...
    highscoretext = font_style.render("HighScore: "+str(highscore), True, white, black)
    lose = font_style.render("Bruh you're too slow!", True, white, black)
    text = font_style.render("Shoot Doge in the head once to kill Or Shoot thrice in his body to kill", True, white,
                             black)
    ...
    while not state:
        ...
        starttime -= 1
        if starttime <= 0:
            sql = "INSERT INTO doge (Name, Score) VALUES (%s, %s)"
            val = (name, score)
            mycursor.execute(sql, val)
            mydb.commit()
            dis.fill(white)
            finalscore = font_style.render("Your Score: " + str(score), True, white, black)
            dis.blit(finalscore, (880, 480))
            game.display.update()
            time.sleep(5)
            game.quit()
            break
        ...
        text4 = font_style.render("time :" + str(starttime), True, textcolour)
...
# main
ch = gui.confirm(text='SELECT A GAME MODE', title='WELCOME TO AIM LAB DOGE VERSION...',
                 buttons=['rampage', 'boss fight'])
if ch == 'rampage':
    kills()
if ch == 'boss fight':
    boss()
